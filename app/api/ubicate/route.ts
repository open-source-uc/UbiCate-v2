...
async function githubFileOperation(url: string, identifier: string, file_places: Places, file_sha: string, operationType: string): Promise<any> {
  try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existsInApproved) {
      return NextResponse.json(
        { message: "¡El lugar ya existe en lugares aprobados!" },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("Concurrent modification")) {
        ...
        return NextResponse.json(
          { message: "La operación no pudo completarse debido a una modificación concurrente. Por favor, inténtelo nuevamente." },
          { status: 409 }  // Conflict
        );
      }
      throw error;
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    try {
      ...
      const { url: approvedPlacesUrl, fileData: approvedPlaces, file_sha: approvedPlacesSha } = await fetchApprovedPlaces();
      ...
      await githubFileOperation(approvedPlacesUrl, placeToApprove.properties.identifier, approvedPlaces, approvedPlacesSha, "APPROVE");

      // 6. Actualizar newPlaces (remover el lugar aprobado)
      newPlaces.features.splice(newPlacesIndex, 1);

      // 7. Guardar cambios en newPlaces
      await githubFileOperation(newPlacesUrl, placeToApprove.properties.identifier, newPlaces, newPlacesSha, "REMOVE_FROM_NEW");
