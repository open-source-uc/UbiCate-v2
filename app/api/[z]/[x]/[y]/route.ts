...
export async function GET(request: NextRequest, { params }: { params: Promise<{ x: string; y: string; z: string }> }) {
  try {
    ...
    if (zNum < 0 || zNum > 22 || xNum < 0 || yNum < 0) {
      return NextResponse.json({ error: "Tile coordinates out of bounds" }, { status: 400 });
    }
    ...
    if (xNum > maxTileIndex || yNum > maxTileIndex) {
      return NextResponse.json({ error: `Tile coordinates out of bounds for zoom level ${zNum}` }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to decompress tile data" }, { status: 500 });
    }
